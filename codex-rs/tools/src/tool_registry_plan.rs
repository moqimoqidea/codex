...
pub fn build_tool_registry_plan(
    ...
    for tool in params.dynamic_tools {
        match dynamic_tool_to_responses_api_tool(tool) {
            Ok(converted_tool) => {
                let handler_name = tool
                    .namespace
                    .as_ref()
                    .map(|namespace| ToolName::namespaced(namespace.clone(), tool.name.clone()))
                    .unwrap_or_else(|| ToolName::plain(tool.name.clone()));
                let spec = if tool.defer_loading {
                    ToolSpec::Function(converted_tool)
                } else if let Some(namespace) = tool.namespace.as_ref() {
                    ToolSpec::Namespace(ResponsesApiNamespace {
